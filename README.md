...
- **Funny propagated-death messages** — when the shared health pool hits 0, whoever's own damage actually emptied it dies normally, but everyone else on the team who goes down purely because the pool did gets a random joke death message instead (there's a large pool of them), since nothing actually hit them.
...
- **"Request slot" ping** — press the (unbound by default) "Request Hovered Slot" key while hovering someone else's locked hotbar slot in any inventory screen to send them a friendly actionbar nudge. Rate-limited per player so it can't be spammed.
- **Teams** — when "Split into teams" is on, sharing (inventory, equipment, hotbar ownership, health, hunger, all of it) is scoped per-team instead of one pool for the whole server. Manage teams with `/symbiote team create|delete|list|join|leave|assign`. With teams off, everyone is effectively on one implicit team - the original server-wide-pool behavior.
...
- **In-game**: press the "Open Symbiote Settings" key (unbound by default — bind it in Controls) to open the settings screen. Anyone can open it to view the current settings, but only the singleplayer host or a server operator can actually save changes. The low-health warning toggle in that same screen is the one exception — it's client-local, so anyone can change their own.
- **Command**: `/symbiote config` shows current settings; `/symbiote config <setting> <value>` changes one (op-only). Settings: `syncArmor`, `syncOffhand`, `enableHotbarOwnership`, `syncHealth`, `syncHunger`, `teamsEnabled` (all booleans).
...
- `create <name>` / `delete <name>` — the implicit `global` team (used when `teamsEnabled` is off) can't be deleted.
- `join <name>` — run by a player, puts them on that team (creating it if needed).
- `leave` — run by a player, returns them to the default team.
- `assign <player> <name>` — puts any player on a team from console or as another player.

Assignments are remembered even while `teamsEnabled` is off (they just aren't used until it's on), and are lost on server restart along with everything else Symbiote tracks in memory. If hotbar ownership is also on, a team is capped at 9 online members the same way the whole server used to be.
...
`TeamManager` decides which `Team` a player resolves to: with `teamsEnabled` off, everyone resolves to the same implicit `global` team regardless of any `/symbiote team` assignment on record, which is exactly the original single-server-pool behavior; with it on, a player resolves to whichever team they last joined/were assigned to (or an auto-created `default` team if never assigned one). Flipping `teamsEnabled`, or changing a player's assignment, re-points every affected online player's `Inventory` at the right team's item list immediately (`TeamManager#reassignInventory`) rather than waiting for a rejoin - equipment and hotbar-ownership/health/hunger don't need this since they already resolve the team fresh on every call/tick.
